...
from suite_trading.domain.market_data import Bar
from suite_trading.utils.data_generation import create_bar, create_bar_series
...
bars = create_bar_series(
    ...
    num_bars=20,
    price_pattern_func=lambda x: linear_function(x=x, start_price=1.0, trend_rate=-0.01),  # (0.01 = 1% increase per bar)
)
...
bars = create_bar_series(
    first_bar = create_bar(is_bullish=True),
    num_bars=100,
    price_pattern_func=sine_wave_function,
)
...
bars = create_bar_series(
    first_bar = create_bar(is_bullish=True),
    num_bars=100,
    price_pattern_func=lambda x: sine_wave_function(x, amplitude=0.02, frequency=0.2),
)
...
combined_pattern_function = lambda x, **kwargs: linear_function(x, trend_rate=-0.0001) + sine_wave_function(x, amplitude=0.01, frequency=0.1) - 1.0
bars = create_bar_series(
    first_bar = create_bar(is_bullish=True),
    num_bars=500,
    price_pattern_func=combined_pattern_function,
)
...
bars = create_bar_series(
    first_bar = create_bar(is_bullish=True),
    num_bars=100,
    price_pattern_func=zig_zag_function,
)
