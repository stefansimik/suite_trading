...
class OrderState(State):
    ...
    PARTIALLY_FILLED = "PARTIALLY_FILLED"  # Order filled partially; some abs_qty remains unfilled
    ...
    FILLED = "FILLED"  # Filled completely; no abs_qty remains
...
class OrderAction(Action):
    ...
    UPDATE = "UPDATE"  # Ask to change price, abs_qty, or other order params
    CANCEL = "CANCEL"  # Ask the broker to cancel the order

    # Fill actions
    PARTIAL_FILL = "PARTIAL_FILL"  # Some abs_qty just filled; some remains unfilled
    FILL = "FILL"  # All remaining abs_qty just filled; order complete
...
def create_order_state_machine(initial_state: OrderState) -> StateMachine[OrderState, OrderAction]:
    ...
    Returns:
        StateMachine[OrderState, OrderAction]: A configured state machine for managing order states.
    """
    transitions = {
