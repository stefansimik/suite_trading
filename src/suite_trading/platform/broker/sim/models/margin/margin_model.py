...
class MarginModel(Protocol):
    ...
    - Initial margin is therefore blocked incrementally per slice, only when absolute
      ...
      implementation may return a zero $Money amount and no new initial margin is blocked.
    ...
    - The current simulation stack assumes a single settlement currency per account; there
      is no automatic FX conversion inside the MarginModel or broker. Any cross-currency
      behavior must be modeled explicitly by the caller or a higher-level component.
    """
