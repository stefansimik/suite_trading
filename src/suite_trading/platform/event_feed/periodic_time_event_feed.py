...
from suite_trading.utils.datetime_tools import require_utc, format_dt, format_range
...
class FixedIntervalEventFeed:
    ...
    def __init__(
        ...
        # Raise: $start_datetime must be timezone-aware UTC to avoid ambiguous scheduling
        require_utc(start_dt)

        # Raise: $interval must be a timedelta to enforce an unambiguous schedule unit
        if not isinstance(interval, timedelta):
            raise ValueError("Cannot call `FixedIntervalEventFeed.__init__` because $interval is not timedelta.")

        # Raise: $interval must be > 0 to make forward progress
        if interval <= timedelta(0):
            raise ValueError("Cannot call `FixedIntervalEventFeed.__init__` because $interval is non-positive.")

        # Raise: $end_datetime (when provided) must be UTC and >= $start_datetime
    ...
    def remove_events_before(self, cutoff_time: datetime) -> None:
        ...
        """
        # Raise: $cutoff_time must be timezone-aware UTC to preserve consistent ordering
