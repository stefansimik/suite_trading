...
class Strategy(ABC):
    ...
    **Example Use Cases:**
    ...
    def last_event_time(self) -> datetime | None:
        ...
        return self._trading_engine._clocks_by_strategy_dict[self].last_event_time
    ...
    def wall_clock_time(self) -> datetime | None:
        ...
        return self._trading_engine._clocks_by_strategy_dict[self].wall_clock_time
    ...
    def on_order_updated(self, order: Order) -> None:
        ...
        """
        logger.debug(f"Strategy named '{self.name}' (class {self.__class__.__name__}) received order update for Order $order_id ('{order.order_id}')")
