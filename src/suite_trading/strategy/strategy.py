...
class Strategy(ABC):
    ...
    **Shared engine timeline:**
    ...
    def __init__(self, name: str) -> None:
        ...
        """
        # Raise: validate $name is a non-empty str
    ...
    def set_trading_engine(self, trading_engine: TradingEngine) -> None:
        ...
        """
        # Raise: $_trading_engine must not be already set
        if self._trading_engine is not None:
            raise RuntimeError("Cannot call `set_trading_engine` because $_trading_engine is already set")
        # Raise: Strategy must be in $state NEW to attach a TradingEngine
        if self.state != StrategyState.NEW:
            raise RuntimeError(f"Cannot call `set_trading_engine` because $state ({self.state.name}) is not NEW. Provide a fresh instance")
    ...
    def _require_trading_engine(self) -> TradingEngine:
        ...
        Notes:
         ...
         Raises:
             RuntimeError: If $_trading_engine is None
        """
        # Raise: TradingEngine must be attached to this Strategy
        if self._trading_engine is None:
            raise RuntimeError("Cannot call `_require_trading_engine` because $_trading_engine is None. Add the Strategy to a TradingEngine using `add_strategy` first")
    ...
    def add_event_feed(
        ...
        *,
        ...
        # Raise: Strategy must be in $state ADDED or RUNNING to add feeds
    ...
    def remove_event_feed(self, feed_name: str) -> None:
        ...
        # Raise: state must be RUNNING to remove feeds
    ...
    def get_broker(self, name: str) -> Broker:
        ...
        # Raise: broker name must be present in the attached TradingEngine
        if name not in engine.brokers:
            raise KeyError(f"Cannot call `get_broker` because Broker named $name ('{name}') is not registered in the attached TradingEngine. Add the broker using `add_broker` first")
    ...
    def submit_order(self, order: Order, broker: Broker) -> None:
        ...
        # Raise: state must be RUNNING to submit orders
        if self.state != StrategyState.RUNNING:
            valid_actions = [a.value for a in self._state_machine.list_valid_actions()]
    ...
    def cancel_order(self, order: Order) -> None:
        ...
        # Raise: state must be RUNNING to cancel orders
        if self.state != StrategyState.RUNNING:
            valid_actions = [a.value for a in self._state_machine.list_valid_actions()]
