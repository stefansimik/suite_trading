...
def create_order_book(
    ...
    """Create a simple OrderBook for tests from raw price/volume pairs.

    This helper is meant for quick test fixtures. Prices and volumes can be
    provided as Decimal, string, or int and will be converted to Decimal.
    ...
    Args:
        ...
        timestamp: Optional UTC timestamp. If not provided, a fixed default
            timestamp is used to keep tests deterministic.
    ...
    Examples:
        ...
        book = TST.order_book.create_order_book(
            instrument=instrument,
            bids=[("99", "10")],
            asks=[("101", "5")],
        )
...
def _parse_price_volume_string(raw_level: str) -> tuple[Decimal, Decimal]:
    ...
    Returns:
        Tuple of (price, volume) as Decimals.
...
def create_order_book_from_strings(
    ...
    This is a convenience wrapper around `create_order_book` for tests that do
    not need to work directly with Decimal values. Each level string must be in
    the "price@volume" format, for example "99@10" or "101@5".
    ...
    Args:
        ...
        timestamp: Optional UTC timestamp for the OrderBook snapshot. If not
            provided, a fixed default timestamp is used to keep tests
            deterministic.
    ...
    Examples:
        ...
        instrument = TST.instrument.create_equity_aapl()
        book = TST.order_book.create_order_book_from_strings(
            instrument=instrument,
            bids=["99@10"],
            asks=["101@5", "102@5"],
        )
        ask_prices = [level.price for level in book.list_asks()]
    """
...
def _parse_price_volume_levels(levels: Iterable[str] | None) -> list[tuple[Decimal, Decimal]]:
    ...
    Returns:
        List of (price, volume) pairs as Decimals. Returns an empty list when
        $levels is None.
