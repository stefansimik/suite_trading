...
class TestOrderBookSimulateFills:
    ...
    def test_buy_consumes_asks_best_first(self):
        """BUY should consume ask levels best-first until abs_qty is met (10@100 then 2@101)."""
        instr = self._instrument()
        ts = self._ts()
        asks = [BookLevel(Decimal("100"), Decimal("10")), BookLevel(Decimal("101"), Decimal("5"))]
        book = OrderBook(instr, ts, bids=(), asks=tuple(asks))

        fills = book.simulate_fills(target_signed_qty=Decimal("12"))
        pairs = [(f.signed_qty, f.price) for f in fills]
    ...
    def test_sell_consumes_bids_best_first(self):
        """SELL should consume bid levels best-first (4@99 then 1@98 to reach abs_qty=5)."""
        instr = self._instrument()
        ts = self._ts()
        bids = [BookLevel(Decimal("99"), Decimal("4")), BookLevel(Decimal("98"), Decimal("7"))]
        book = OrderBook(instr, ts, bids=tuple(bids), asks=())

        fills = book.simulate_fills(target_signed_qty=-Decimal("5"))
        pairs = [(f.signed_qty, f.price) for f in fills]
    ...
    def test_price_filters_min_max_respected(self):
        ...
        fills = book.simulate_fills(target_signed_qty=Decimal("30"), max_price=Decimal("101"))
        pairs = [(f.signed_qty, f.price) for f in fills]
    ...
    def test_negative_prices_allowed(self):
        ...
        fills = book.simulate_fills(target_signed_qty=Decimal("1"))
        assert [(f.signed_qty, f.price) for f in fills] == [(Decimal("1"), Decimal("-5"))]
